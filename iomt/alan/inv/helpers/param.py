...
def main_big():
    ...
    for epoch in range(num_epochs):
        ...
        loss = F.mse_loss(pred_data, syn_data)
        loss.backward()
        optimizer.step()
        if epoch % 10 == 0 or epoch == num_epochs - 1:
            print(f"Epoch {epoch} | Loss: {loss.item():.6f}")
            print(f"mu: {source_model.mu.data.cpu().numpy()}, "
    ...
    final_res_msg = get_msg(
        source_model.mu, source_model.sig, source_model.peak_time,
        source_model.freq, source_model.scale, 'Final result:'
    )
...
def main():
    ...
    for j in range(output.shape[1]):
        plt.plot(output[0, j, :].detach().cpu().numpy(), label=f"Source {j}", linestyle=style[j])
...
def main2d():
    ...
    # Run a forward pass.
    output = source_module.forward().detach().cpu().numpy() # shape: [1, num_sources, nt]
    print("Output shape:", output.shape)
    
    output = output.squeeze().reshape(grid_size, grid_size, nt)  # shape: [grid_size, grid_size, nt]
