...
from  mh.typlotlib import get_frames_bool, save_frames, bool_slice

class BatchedRiemannLiouvilleFractionalIntegral(nn.Module):
    def __init__(self, alphas, dt, max_length, delta=1e-8, gamma=0.1):
        """
        ...
        for a in self.alphas:
            a_val = float(a.item())
            if abs(a_val) < delta:  # Special case: alpha == 0 => identity operator.
...
if __name__ == "__main__":
    ...
    epsilon = 0.1
    N = int(1 / epsilon)    
    alphas = [epsilon * i for i in range(N)] + [1.0]
    alphas.append(1.0)
    alphas.insert(0, 0.0)
    
    # Instantiate the batched fractional integral operator.
    frac_int = BatchedRiemannLiouvilleFractionalIntegral(alphas, dt, max_length, gamma=0.1)
    ...
    def plotter(*, data, idx, fig, axes):
        ...
        plt.plot(t.numpy(), f.numpy(), 'r--', label="Input f(t)")
        plt.plot(t.numpy(), y1_analytic.numpy(), 'k--', label="Analytic Integral (dt*cumsum)")
        plt.plot(t.numpy(), y[idx[0]].detach().numpy(), label=f"Output for alpha={alphas[idx[0]]}")
    ...
    save_frames(frames, path="mine", duration=1000) 