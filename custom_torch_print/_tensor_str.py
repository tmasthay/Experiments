...
class _Formatter:
        ...
        if not self.floating_dtype:
            ...
            if self.int_mode:
                ...
                if (
                    ...
                    self.sci_mode = True
                    for value in nonzero_finite_vals:
                        value_str = f"{{:.{PRINT_OPTS.precision}e}}".format(
                            value
                        )
                        self.max_width = max(self.max_width, len(value_str))
                else:
                ...
                if (
                    ...
                    for value in nonzero_finite_vals:
                        value_str = f"{{:.{PRINT_OPTS.precision}f}}".format(
                            value
                        )
                        self.max_width = max(self.max_width, len(value_str))
    ...
    def format(self, value):
        if self.floating_dtype:
            if self.sci_mode:
                ret = f"{{:{self.max_width}.{PRINT_OPTS.precision}e}}".format(
                    value
                )
            elif self.int_mode:
                ret = f"{value:.0f}"
...
def _tensor_str_with_formatter(
    self, indent, summarize, formatter1, formatter2=None
):
    dim = self.dim()
...
def _add_suffixes(tensor_str, suffixes, indent, force_newline):
    ...
    for suffix in suffixes:
        ...
        if (
            force_newline
            or last_line_len + suffix_len + 2 > PRINT_OPTS.linewidth
        ):
            tensor_strs.append(",\n" + " " * indent + suffix)
...
def get_summarized_data(self):
    ...
    if not PRINT_OPTS.edgeitems:
        ...
        start = [self[i] for i in range(0, PRINT_OPTS.edgeitems)]
            ...
            self[i] for i in range(len(self) - PRINT_OPTS.edgeitems, len(self))
        ]
...
def _str_intern(inp, *, tensor_contents=None):
    if torch._C._functorch.is_functorch_wrapped_tensor(inp):
        return _functorch_wrapper_str_intern(
            inp, tensor_contents=tensor_contents
        )
    is_plain_tensor = (
        type(inp) is torch.Tensor or type(inp) is torch.nn.Parameter
    )
    if inp.is_nested:
    ...
    if self.is_sparse:
        ...
        if (
            ...
            suffixes.append(
                "zero_point=" + str(self.q_per_channel_zero_points())
            )
...
def _str(self, *, tensor_contents=None):
    with torch.no_grad(), torch.utils._python_dispatch._disable_current_modes():
        ...
        if PRINT_OPTS.callback is None:
            return _str_intern(self, tensor_contents=tensor_contents)
        else:
            return PRINT_OPTS.callback(self)
