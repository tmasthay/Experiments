...
# base.taskMgr.add(tk_update, 'tk_update')
# base.run()

def tk_update(task):
    base.tkRoot.update()
    taskMgr.doMethodLater(0.01, tk_update, 'tk_update')  # Update every 0.01 seconds (100 FPS)
    return task.done

taskMgr.doMethodLater(0.01, tk_update, 'tk_update')  # Start the task with the specified delay
base.run()
